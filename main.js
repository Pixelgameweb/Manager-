...
function deleteExpense(element) { // Тут у меня функция удаления таблицы
    ...
    const category = parentRow.querySelector('.purcashes__td:nth-child(2)').innerText; // Получаю каждую коатегорию 
    const price = parseFloat(parentRow.querySelector('.purcashes__td:nth-child(3)').innerText); // Получаю каждую цену и преобразую ее в число 
    ...
    parentRow.remove(); // Удаляем Элемент таблицы 
}
...
function addExpense(name, category, price) { // Функция добавления элемментов таблицы
    ...
    const td = document.createElement('tr');// Создаем константу которая будет создавать элемент таблицы
